<template>
  <n-card :bordered="false" class="proCard">
    <n-form label-placement="left" inline>
      <n-form-item label="名称">
        <n-input v-model:value="state.queryParams.name" class="w180" placeholder="输入角色名称" clearable />
      </n-form-item>
  ...
  <RoleEdit ref="roleEditRef" :title="state.editTitle" @handleQuery="reloadTable"></RoleEdit>
</template>
...
// 新增
const handleAdd = () => {
  state.editTitle = '添加角色'
  roleEditRef.value?.openModal({})
}
...
const handleEdit = (record) => {
  ...
  roleEditRef.value?.openModal(record)
}
