		...
		<n-form label-placement="left" inline>
			...
			<n-space>
				<n-button type="primary" @click="reloadTable"
					v-if="hasPermission(['service/recharge/auth/getRechargeTemplateList'])">查询</n-button>
				<!-- <n-button @click="resetQuery">重置</n-button> -->
				<n-button type="primary" @click="handleAdd" v-if="hasPermission(['service/recharge/auth/addRechargeTemplate'])">
					<template #icon>
						<n-icon>
							<PlusOutlined />
						</n-icon>
...
const columns = [
	...
	},
	{
		...
		render(row: any) {
			...
			return row.sales_price ? Number(row.sales_price / 100) : ''
		}
	},
	{
		title: '排序',
...
const authWidth = () => {
	if (hasPermission(['service/recharge/auth/updateRechargeTemplate']) && hasPermission(['service/recharge/auth/deleteRechargeTemplate'])) {
		return 100
	}
	if (hasPermission(['service/recharge/auth/updateRechargeTemplate']) || hasPermission(['service/recharge/auth/deleteRechargeTemplate'])) {
		return 60
	...
	return [
		{
			...
			auth: ['service/recharge/auth/updateRechargeTemplate'],
		},
		{
			label: '删除',
			type: 'error',
			...
			auth: ['service/recharge/auth/deleteRechargeTemplate'],
		},
	]
}
