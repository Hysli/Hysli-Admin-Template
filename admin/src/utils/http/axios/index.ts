...
const transform: AxiosTransform = {
  ...
  transformRequestData: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    //  这里 code，result，message 为 后台统一的字段，需要修改为项目自己的接口返回格式
    ...
    if (isShowMessage) {
      if (hasSuccess && (successMessageText || isShowSuccessMessage)) {
        ...
        // errorMessageMode=‘custom-modal’的时候会显示 modal 错误弹窗，而不是消息提示，用于一些比较重要的错误
        $dialog.info({
          title: '提示',
          content: msg,
          positiveText: '确定',
    ...
    // 接口请求成功，直接返回结果
    if (code == ResultEnum.SUCCESS_TIPS) {
      $message.success(msg)
    }
    if (code === ResultEnum.SUCCESS_TIPS || code == ResultEnum.SUCCESS) {
      return res.data
    }
